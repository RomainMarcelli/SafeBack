// Tests unitaires pour valider le comportement de `db` et prévenir les régressions.
...
describe("db helpers", () => {
  ...
  it("disableAllLiveShareSessions disables active sharing for current user", async () => {
    setResult("sessions", "select", {
      data: [{ id: "session-1" }, { id: "session-2" }],
      error: null
    });

    const count = await disableAllLiveShareSessions();
  ...
  it("createIncidentReport inserts payload for authenticated user", async () => {
    setResult("incident_reports", "single", {
      data: { id: "incident-1", user_id: "user-1", incident_type: "sos", details: "SOS" },
      error: null
    });
    ...
    const created = await createIncidentReport({
      ...
      incident_type: "sos",
      severity: "high",
      occurred_at: "2026-02-14T20:00:00.000Z",
    ...
    const insertCall = state.calls.find(
      (call) => call.table === "incident_reports" && call.action === "insert"
    );
  ...
  it("listIncidentReports returns ordered list", async () => {
    setResult("incident_reports", "limit", {
      data: [{ id: "incident-2" }, { id: "incident-1" }],
      error: null
    });

    await expect(listIncidentReports(20)).resolves.toEqual([
      { id: "incident-2" },
      { id: "incident-1" }
    ]);
  });
