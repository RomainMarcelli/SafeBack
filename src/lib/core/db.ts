// Couche d'accès Supabase pour les entités cœur (profil, contacts, sessions, localisations...).
...
import type { SharedLocationPoint } from "../trips/liveShare";
export type {
  Contact,
  FavoriteAddress,
  IncidentReport,
...
export async function disableAllLiveShareSessions(): Promise<number> {
  const session = await supabase.auth.getSession();
...
export async function createIncidentReport(payload: {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      incident_type: payload.incident_type ?? "sos",
...
export async function listIncidentReports(limit = 100): Promise<IncidentReport[]> {
  const session = await supabase.auth.getSession();
  const userId = session.data.session?.user.id;
  if (!userId) {
    throw new Error("Utilisateur non'authentifie.");
